...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        cptWithEvi = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for var in eliminationOrder:
            cptWithEvi, factor = joinFactorsByVariable(cptWithEvi, var)
            if len(factor.unconditionedVariables()) > 1:
                factor = eliminate(factor, var)
                cptWithEvi.append(factor)
        newFactors = joinFactors(cptWithEvi)
        return normalize(newFactors)

...
def constructBayesNet(gameState):
    ...
    xyPos = [X_POS_VAR, Y_POS_VAR]
    houseVar = [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]
    tempObsVars = []
    tempObsVars.append(X_POS_VAR)
    tempObsVars.append(Y_POS_VAR)
    tempObsVars.append(FOOD_HOUSE_VAR)
    tempObsVars.append(GHOST_HOUSE_VAR)
    for housePos in gameState.getPossibleHouses():
        for tempObsPos in gameState.getHouseWalls(housePos):
            tempObsVar = OBS_VAR_TEMPLATE % tempObsPos
            obsVars.append(tempObsVar)
            tempObsVars.append(tempObsVar)
    for xy in xyPos:
        for house in houseVar:
            edges.append((xy, house))
    for house in houseVar:
        for obs in obsVars:
...
def fillObsCPT(bayesNet, gameState):
        ...
        for wall in walls:
            ...
            for assignment in assignments:
                ...
                if house == topRightPos:
                    house = "topRight"
                if house != str(assignment[GHOST_HOUSE_VAR]) and house != str(assignment[FOOD_HOUSE_VAR]):
                    if assignment[obsVar] == NO_OBS_VAL:
                        prob = 1
                    else:
                        prob = 0
                if house == str(assignment[FOOD_HOUSE_VAR]):
    ...
    # for obs in bayesNet.variableDomainsDict():
    #     obsPos = gameState.getObservedPositions()
    # print(obs)
    # print(bottomLeftPos)
    # print(topLeftPos)
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        factor = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)
        assignments = factor.getAllPossibleAssignmentDicts()

        for assignment in assignments:
            if assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                prob_right = factor.getProbability(assignment)
                print(assignment)
        for assignment in assignments:
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                prob_left = factor.getProbability(assignment)
        # print(evidence)
        # condEvidenceRight = dict(evidence)
        # condEvidenceRight.update({FOOD_HOUSE_VAR: TOP_RIGHT_VAL, GHOST_HOUSE_VAR: TOP_LEFT_VAL})
        # prob_right = factor.getProbability(condEvidenceRight)
        # condEvidenceLeft =dict(evidence)
        # condEvidenceLeft.update({FOOD_HOUSE_VAR: TOP_LEFT_VAL, GHOST_HOUSE_VAR: TOP_RIGHT_VAL});   
        # prob_left = factor.getProbability(condEvidenceLeft)
        # print(prob_left)
        # print(prob_right)
        # print(WON_GAME_REWARD)
        # print(GHOST_COLLISION_REWARD)
        leftExpectedValue = prob_left*WON_GAME_REWARD + prob_right * GHOST_COLLISION_REWARD
        rightExpectedValue = prob_right * WON_GAME_REWARD + prob_left*GHOST_COLLISION_REWARD
